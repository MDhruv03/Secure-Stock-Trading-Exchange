document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (result.success) {
                    ...
                    document.getElementById('current-user-balance').textContent = result.balance || '10000.00';
                    ...
                    userId = result.user_id || 12345; // fallback for demo
    ...
    function loadCryptoDemo() {
        adminContent.innerHTML = `
            <div class="space-y-6">
                ...
                <!-- Real-time Crypto Visualization -->
                <div class="bg-gray-900 border border-gray-700 rounded-lg p-6">
                    <h2 class="text-lg font-semibold text-green-400 mb-4">CRYPTOGRAPHIC_PROCESS_VISUALIZATION</h2>
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div class="bg-gray-800 p-4 rounded-lg text-center">
                            ...
                            <h3 class="text-blue-400 font-semibold">DATA</h3>
                            <p class="text-xs text-gray-400 mt-1">Original data to encrypt</p>
                        </div>
                        ...
                        <div class="bg-gray-800 p-4 rounded-lg text-center">
                            ...
                            <h3 class="text-purple-400 font-semibold">ENCRYPTED</h3>
                            <p class="text-xs text-gray-400 mt-1">AES-256-GCM encrypted</p>
                        </div>
                        <div class="bg-gray-800 p-4 rounded-lg text-center flex flex-col items-center justify-center">
                            <div class="w-8 h-8 bg-gray-600 rounded-full flex items-center justify-center mb-2">
                                <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="bg-gray-800 p-4 rounded-lg text-center md:col-start-4">
                            ...
                            <h3 class="text-green-400 font-semibold">SIGNED</h3>
                            <p class="text-xs text-gray-400 mt-1">RSA-2048 signature</p>
                        </div>
                    </div>
                </div>
                
                <!-- Crypto Information Panel -->
                <div class="bg-gray-900 border border-gray-700 rounded-lg p-6">
                    <h2 class="text-lg font-semibold text-green-400 mb-4">CRYPTOGRAPHIC_INFORMATION</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="bg-gray-800 p-4 rounded-lg">
                            <h3 class="text-blue-400 font-semibold mb-2">AES-256-GCM</h3>
                            <p class="text-sm text-gray-300">Advanced Encryption Standard with Galois/Counter Mode provides authenticated encryption with high security.</p>
                            <div class="mt-2 text-xs text-gray-400">
                                <p>• 256-bit keys for maximum security</p>
                                <p>• Authenticated encryption</p>
                                <p>• Fast performance</p>
                            </div>
                        </div>
                        <div class="bg-gray-800 p-4 rounded-lg">
                            <h3 class="text-purple-400 font-semibold mb-2">RSA-2048</h3>
                            <p class="text-sm text-gray-300">Rivest-Shamir-Adleman algorithm with 2048-bit keys for digital signatures and key exchange.</p>
                            <div class="mt-2 text-xs text-gray-400">
                        ...
                        <div class="bg-gray-800 p-4 rounded-lg">
                            <h3 class="text-yellow-400 font-semibold mb-2">Merkle Trees</h3>
                            <p class="text-sm text-gray-300">Hash-based data structures for efficient and secure verification of transaction integrity.</p>
                            <div class="mt-2 text-xs text-gray-400">
                                <p>• Tamper-evident</p>
                                <p>• Efficient verification</p>
                                <p>• Cryptographic proofs</p>
                            </div>
                        </div>
                        <div class="bg-gray-800 p-4 rounded-lg">
                            <h3 class="text-green-400 font-semibold mb-2">Homomorphic Encryption</h3>
                            <p class="text-sm text-gray-300">Encryption scheme that allows computation on encrypted data without decrypting it.</p>
                            <div class="mt-2 text-xs text-gray-400">
                                <p>• Compute on encrypted data</p>
                                <p>• Privacy-preserving</p>
                                <p>• Secure analytics</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    function initCryptoDemo() {
        ...
        document.getElementById('generate-crypto-demo-btn').addEventListener('click', () => {
            document.getElementById('plaintext').value = JSON.stringify({
                order_id: `ORD-${Math.floor(Math.random() * 1000)}`,
                symbol: "BTC",
                quantity: parseFloat((Math.random() * 10).toFixed(2)),
                price: parseFloat((Math.random() * 50000 + 30000).toFixed(2))
            });
            document.getElementById('data-to-sign').value = JSON.stringify({
                tx_id: `TX-${Math.floor(Math.random() * 1000)}`,
                timestamp: new Date().toISOString(),
                user_id: Math.floor(Math.random() * 10000)
            });
            document.getElementById('transaction-data').value = JSON.stringify({
                tx_id: `TX-${Math.floor(Math.random() * 1000)}`,
    ...
    function loadLogs() {
        adminContent.innerHTML = `
            <div class="space-y-6">
                ...
                <div class="bg-gray-900 border border-gray-700 rounded-lg p-6 hover:border-green-500/50 transition-colors">
                    <h2 class="text-lg font-semibold text-green-400 mb-4">USER_ACTIVITY_LOGS</h2>
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody class="text-gray-300">
                                <tr class="border-b border-gray-800">
                                    ...
                                    <td class="py-2 text-blue-400">USER_LOGIN</td>
                                    <td class="py-2">User demo_user logged in successfully</td>
                                    <td class="py-2 text-gray-400">2024-01-15 10:30:42</td>
                                </tr>
                                <tr class="border-b border-gray-800">
                                    <td class="py-2 text-green-400">002</td>
                                    <td class="py-2 text-purple-400">ORDER_PLACED</td>
                                    <td class="py-2">Secure order placed for 0.5 BTC with cryptographic protection</td>
                                    <td class="py-2 text-gray-400">2024-01-15 11:15:23</td>
                                </tr>
                                <tr class="border-b border-gray-800">
                                    <td class="py-2 text-green-400">003</td>
                                    <td class="py-2 text-yellow-400">ORDER_EXECUTED</td>
                                    <td class="py-2">Order for BTC executed successfully with encryption verified</td>
                                    <td class="py-2 text-gray-400">2024-01-15 12:45:10</td>
                                </tr>
                                <tr class="border-b border-gray-800">
                                    <td class="py-2 text-green-400">004</td>
                                    <td class="py-2 text-green-400">USER_LOGOUT</td>
                                    <td class="py-2">User demo_user logged out</td>
    ...
    function connectWebSocket(userId) {
        ...
        // In a real implementation, we would use the real WebSocket URL
        // For this demo, we'll fake the WebSocket connection
        ws = {
    ...
    function simulateWebSocketMessages() {
        ...
        setInterval(() => {
            const events = [
                { type: 'API_CALL', desc: 'API endpoint /api/trading/orders accessed' },
                { type: 'DATA_ENCRYPTED', desc: 'Order data encrypted successfully' },
                { type: 'SIGNATURE_VERIFIED', desc: 'Digital signature verified' },
                { type: 'MFA_CHECK', desc: 'Multi-factor authentication verified' }
            ];
            const randomEvent = events[Math.floor(Math.random() * events.length)];
            addSecurityEvent(randomEvent.type, randomEvent.desc);
        }, 10000);
    }
    ...
    function updateMarketData() {
        ...
        if (marketData) {
            ...
            // For demo, we'll just update with random changes
            const rows = marketData.querySelectorAll('tr');
            rows.forEach((row, index) => {
                const cells = row.querySelectorAll('td');
                if (cells.length > 1) {
                    // Get current price and randomly adjust
                    const currentPriceText = cells[1].textContent;
                    ...
                    // Update percentage change with color based on positive/negative
                    cells[2].textContent = percentChange > 0 ? '+' + percentChange + '%' : percentChange + '%';
                    cells[2].className = percentChange >= 0 ? 'py-2 text-right text-green-400' : 'py-2 text-right text-red-400';
                    
                    // Update VWAP similarly
                    const currentVwapText = cells[4].textContent;
                    const currentVwap = parseFloat(currentVwapText.replace('$', '').replace(',', ''));
                    const vwapChange = (Math.random() - 0.5) * 50;
                    const newVwap = Math.max(0.01, currentVwap + vwapChange);
                    cells[4].textContent = '$' + newVwap.toFixed(2);
                }
            });
        }
    }
    ...
    // Initialize demo data for system status
    setTimeout(() => {
